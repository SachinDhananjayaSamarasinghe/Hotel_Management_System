...
$query_CMSPage = "SELECT * FROM pages1 ORDER BY `Timestamp` ASC";
$CMSPage = mysql_query($query_CMSPage, $localhost) or die(mysql_error());
...
mysql_select_db($database_localhost, $localhost);
$query_PageContent = sprintf("SELECT * FROM pages1 WHERE ID = %s", GetSQLValueString($colname_PageContent, "int"));
$PageContent = mysql_query($query_PageContent, $localhost) or die(mysql_error());
...
<html>
<head>
<title>More About Food & Beverages </title>
<meta charset="UTF-8">
...
<body>
	<div class="w3-third">
        ...
        <?php if ($totalRows_CMSPage > 0) { // Show if recordset not empty ?>
          <?php do { ?>
          <p><a href="SubPage1.php?ID=<?php echo $row_CMSPage['ID']; ?>" class="w3-hover-red"><?php echo $row_CMSPage['PageName']; ?></a></p>
